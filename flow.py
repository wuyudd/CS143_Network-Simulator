...
class FlowState(object):
    SLOWSTART_INIT = "ss0" # detect the threshold
    SLOWSTART = "ss" # slow start
    FRFR = "frfr" # fast retransmit and fast recovery
    CA = "ca" # congestion avoidance


class Flow(object):
    def __init__(self, id, src, dest, size, start_time, packet_size, tcp_name):
        self.id = id # flow ID
        self.src = src # the source of flow
        self.dest = dest # the destination of flow
        self.size = size # data amount
        self.start_time = start_time # start time
        self.pkt_pool = {} # all the packets generated by the flow
        self.sending_queue = collections.deque() # packets waiting to be sent
        self.timeout_queue = collections.deque() # packets need to be retransmitted immediately
        self.total_number_of_packet = 0 # the number of packets generated by the flow
        self.tcp_name = tcp_name # the chosen tcp congestion algorithm
        self.recieve_ack_flag = False # whether receive a valid ack before time out
        self.expected_timeout = None # expected timestamp for time out event, used to check if the time out event is canceled or validate
        self.rtt = 0.2
        self.finished = 0 # whether all the packets of the flow are sent successfully

        self.plot_window_size_timestamp = []  # timestamp for plot window size and round trip time
        self.plot_window_size = [] # record window size at the corresponding timestamp for plotting
        self.plot_rtt = [] # round trip time at the corresponding timestamp for plotting

        # for reno initialization
        self.curr_state = FlowState.SLOWSTART_INIT # current state of reno, initial state is slow start init
        self.window_size = 1
        self.prev_window_size = 0 # previous window size
        self.three_dup_flag = False # whether received three duplicate acks or not
        self.num_on_flight_pkt = 0 # number of outstanding packets
        self.dup_ack_count = 1 # counter for duplicate acks
        self.timeout_flag = False # if the flow is time out
        self.ss_threshold = float('Inf')  # threshold
        self.last_ack = Packet(self.id+'pkt-1ack-1', "data_ack", global_consts.ACKSIZE, self.src, self.dest)  # last ack received, initialized as a zombie pkt
        ...
        self.fast_window_size_update_flag = True # mark for update window size every other round trip time
        self.temp_rtt = 0 # record current round trip time
